...
ipca_diffusion %>%
  hchart("area", hcaes(x = date, y = value, group = variable), color = colors[1]) %>%
  ftplot(
...
hchart(ipca_trend, "column", hcaes(x = date, y = `Month over Month (%)`),
       color = colors[1], name = "Month over Month", showInLegend = TRUE) %>%
  hc_add_series(ipca_trend, "line", hcaes(x = date, y = `Year over Year (%)`),
...
ipca_region %>%
  ...
  hc_rangeSelector(selected = 0, enabled = TRUE)
```
...
testando <- highchart() %>%
  ...
  hc_series(
    list(
      ...
      keys = c("name", "y", "color"),
      data = list(
        list("Employee in the private sector", 39.870, "#282f6b"),
        list("Domestic worker", 4.902, "#b22200"),
        list("Employee in the public sector", 12.162, "#eace3f"),
        list("Auxiliary family worker", 2.051, "#224f20"),
        list("Employer", 3.922, "#5f487c"),
        list("Own account", 23.272, "#b35c1e")
      ),
      dataLabels = list(
        enabled = TRUE,
        format = "{point.label}"
...
# teste = employment_category %>%
#   hchart("item", hcaes(y = value, color = variable), color = colors) %>%
#   ftplot(
...
# hcout <-  highchart() %>%
#   hc_title(
#     text = "Employment by category",
#     style = list(fontSize = "15px")
#   ) %>% 
#   hc_chart(
#     data = employment_category$value,
#     type = "item"
#   ) %>% 
#   hc_xAxis(
#     categories = employment_category$variable
#   ) %>% 
#   hc_yAxis(
#     visible = TRUE
#   ) %>% 
